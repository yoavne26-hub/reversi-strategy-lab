...
# In-memory game state is suitable for the current single-instance Render demo.
# A durable multi-instance production app should move this to Redis or a database.
GAMES = {}
LAB_BENCHMARK_JOBS = {}
...
def _store_new_game(state):
    previous_game_id = session.get("game_id")
    if previous_game_id:
        GAMES.pop(previous_game_id, None)
...
def api_new():
    ...
    if mode == "pvp":
        state = _store_new_game(GameState(
            human_color=BLACK,
            ai_strategy=None,
    ...
    state = _store_new_game(GameState(human_color=human_color, ai_strategy=strategy, enable_undo=False))
    return jsonify({"ok": True, "state": state.as_dict()})
