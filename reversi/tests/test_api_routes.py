...
def _active_game_for_client(client):
    with client.session_transaction() as sess:
        game_id = sess.get("game_id")
    assert game_id in routes.GAMES
    return routes.GAMES[game_id]
...
def test_api_new_accepts_advanced_difficulty():
    ...
    assert isinstance(_active_game_for_client(client).ai_strategy, HybridHeuristicStrategy)
...
def test_api_state_without_active_game_returns_controlled_error():
    client = app.test_client()
    ...
    assert res.status_code == 400
    assert data == {"ok": False, "error": "No active game"}


def test_api_games_are_scoped_to_browser_session():
    first_client = app.test_client()
    second_client = app.test_client()

    first_res = first_client.post("/api/new", json={"mode": "pva", "human": "black", "difficulty": "easy"})
    second_res = second_client.post("/api/new", json={"mode": "pva", "human": "white", "difficulty": "medium"})

    assert first_res.status_code == 200
    assert second_res.status_code == 200

    with first_client.session_transaction() as first_session:
        first_game_id = first_session["game_id"]
    with second_client.session_transaction() as second_session:
        second_game_id = second_session["game_id"]

    assert first_game_id != second_game_id
    assert routes.GAMES[first_game_id].human_color == 1
    assert routes.GAMES[second_game_id].human_color == -1

    first_state = first_client.get("/api/state").get_json()["state"]
    second_state = second_client.get("/api/state").get_json()["state"]

    assert first_state["human_color"] == 1
    assert second_state["human_color"] == -1
