...
This project should be deployed on Render as a **Flask Web Service**, not as a Static Site.

A root-level `render.yaml` is included with the deployment configuration. If configuring the service manually in Render, use:
...
The online version uses per-browser-session game state for demo use, so different visitors do not overwrite each other's active games in a single Render instance. The current storage is still in memory; a future multi-instance or persistent production version should move active games and benchmark jobs to Redis, a database, or another external store.
